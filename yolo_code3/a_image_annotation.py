...
def create_marked_images():
    ...
    for image in x[:rng_train]:
        t = threading.Thread(target=worker, args=(image, "yolo_code3/data/train_data/full_size"))
        t.start()
    for image in x[rng_train:rng_train+rng_val]:
        t = threading.Thread(target=worker, args=(image, "yolo_code3/data/val_data/full_size"))
        t.start()
    for image in x[rng_train+rng_val:]:
        t = threading.Thread(target=worker, args=(image, "yolo_code3/data/test_data/full_size"))
        t.start()
    print("done")
