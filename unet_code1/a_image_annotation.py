...
def mark_img(img_path: str, dest_dir=None):
    ...
    full_img = os.path.join(dest_dir, "images", "_".join(img_path.split("/")[1:]))
    color_mask_img = os.path.join(dest_dir, "color_masks", "_".join(img_path.split("/")[1:]))
    mask_img = os.path.join(dest_dir, "masks", "_".join(img_path.split("/")[1:]))
...
def create_marked_images():
    ...
    for image in x[:rng_train]:
        t = threading.Thread(target=worker, args=(image, "unet_code1/data/train_data/full_size"))
        t.start()
    for image in x[rng_train:rng_train+rng_val]:
        t = threading.Thread(target=worker, args=(image, "unet_code1/data/val_data/full_size"))
        t.start()
    for image in x[rng_train+rng_val:]:
        t = threading.Thread(target=worker, args=(image, "unet_code1/data/test_data/full_size"))
        t.start()
    print("done")
