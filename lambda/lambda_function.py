...
YOLO_MODEL_FILE = os.environ["YOLO_MODEL_FILE"]
s3 = boto3.client("s3")
...
def save_results_all(image, y_pred_unet0, y_pred_unet1, y_pred_yolo, save_image_path):
    ## i - m - y
    line = np.ones((H, 10, 3)) * 128
    ...
    """ Predicted Mask """
    y_pred_yolo = np.expand_dims(y_pred_yolo, axis=-1)  ## (512, 512, 1)
    y_pred_yolo = np.concatenate([y_pred_yolo, y_pred_yolo, y_pred_yolo], axis=-1)  ## (512, 512, 3)
    y_pred_yolo = y_pred_yolo * 255

    cat_images = np.concatenate([image, line, y_pred_unet0, line, y_pred_unet1, line, y_pred_yolo], axis=1)
    image_bytes = cv2.imencode(".jpg", cat_images)[1].tobytes()
    put_img_to_s3_object(S3_BUCKET_NAME, save_image_path, image_bytes)
...
def validate_input(event):
    ...
    model_type_choices = ["unet0", "unet1", "yolo", "all"]
...
def get_unet0_output(s3_input_file, save=True):
    ...
    with CustomObjectScope(
        ...
        model = tf.keras.models.load_model(temp_file.name, safe_mode=False)
...
def get_yolo_output(s3_input_file, save=True):
    # result_file_name = s3_input_file.split("/")[-1]
    # result_file = f"lambda_api_predict/yolo/{result_file_name}"
    # x0 = np.frombuffer(get_s3_object_to_buffer(S3_BUCKET_NAME, s3_input_file), np.uint8)
    ...
    # object_buffer = get_s3_object_to_buffer(S3_BUCKET_NAME, YOLO_MODEL_FILE)
    # temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".keras")
    # temp_file.write(object_buffer)
    
    # model = YOLO(temp_file.name)
    # y_pred = model.predict(x0, conf=0.1, visualize=False, save=False)
    
    # y_pred[0].save(result_file_name)
    # y_pred = cv2.imread(result_file_name, cv2.IMREAD_COLOR + cv2.IMREAD_IGNORE_ORIENTATION)
    ...
    return get_unet1_output(s3_input_file, save)
...
def get_all_output(s3_input_file, save=True):
    ...
    y_pred_yolo = get_yolo_output(s3_input_file, save=False)
    save_results_all(image, y_pred_unet0, y_pred_unet1, y_pred_yolo, result_file)
    result_image_url = get_img_public_url(S3_BUCKET_NAME, result_file)
    return result_image_url
...
def predict_output(s3_input_file, model_type):
    if model_type == "unet0":
        ...
        result_image_url = get_yolo_output(s3_input_file)
    elif model_type == "all":
